...
class HAMpedes(BaseImageDataset):

    dataset_dir = 'HAMpedes'

    def __init__(self, root='', test_path = '', verbose=True):
        super(HAMpedes, self).__init__()
        self.dataset_dir = osp.join(root)
        self.img_dir = op.join(self.dataset_dir, 'HAMpedes')
        self.img_test_dir = op.join('./data/cuhkpedes/imgs') 

        self.anno_path = op.join('./data/cuhkpedes', 'reid_raw.json')
        self._check_before_run()

        self.anno_path_train = './data/HAM_pedes/random100w_2HAMcaptions_new.json'
        self._check_before_run()
