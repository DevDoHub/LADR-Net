...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: Tuple[int, int], patch_size: int, stride_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.class_embedding = nn.Parameter(scale * torch.randn(width))#代表“分类标记”（class token）
        ...
        self.temperature_ws = nn.ParameterList([nn.Parameter(torch.ones(1)) for _ in range(self.transformer.layers)])
        self.temperature_bs = nn.ParameterList([nn.Parameter(torch.zeros(1)) for _ in range(self.transformer.layers)])
        self.softplus = nn.Softplus()

    def forward(self, x: torch.Tensor):
        x = self.conv1(x)  # shape = [*, width, grid, grid] 这和传统 ViT 的 patch embedding 是等价的，但实现上用的是卷积而不是 reshape+linear。
        ...
        x = self.transformer(x)
        sw = self.temperature_ws[1].to(x.dtype).to(x.device)
        sb = self.temperature_bs[1].to(x.dtype).to(x.device)
        x = x * self.softplus(sw) + sb#让模型自动学习总输出的“温度”与偏置，有助于特征分布自适应，提升模型表达能力
        ...
        if self.proj is not None:
            x = x @ self.proj#线性变换 self.proj.size() torch.Size([768, 512]) x.size() torch.Size([64, 193, 768])
    
        return x#x.size() torch.Size([64, 193, 512])
