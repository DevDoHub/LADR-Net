...
def do_train(cfg,
    ...
    # 初始化评估器和梯度缩放器
    evaluator = Evaluator(query_loader, gallery_loader)
    ...
    for epoch in range(1, epochs + 1):
        ...
        #         # feat, score,  local_feat_all, text_embeds_s, text_score, semantic_dummy= outputs
        #         feat, score,  local_feat_all, text_embeds_s = outputs
                
        #         # 修改损失函数调用，确保所有输出都参与计算
        #         loss = loss_fn(
        #             score, feat, 
        #             target, text_embeds_s, target_cam, epoch,
        #             local_feat_all  # 添加之前未使用的输出
        #         )
        ...
        #                 world_size = dist.get_world_size()
        #                 logger.info(
        #                     "Rank[{}/{}] Epoch[{}] Iter[{}/{}] Loss: {:.3f}, Acc: {:.3f}, Base Lr: {:.2e}".format(
