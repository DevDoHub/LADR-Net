    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, instruction, vid, target_cam, target_view) in enumerate(train_loader):
            ...
            with amp.autocast(enabled=True):
                ...
                feat, score, _, text_embeds_s = model(img, instruction, label=target, cam_label=target_cam, view_label=target_view )
                ...
                if dist.get_rank() == 0:
                    ...
                    for n_iter, (img, vid, camid, camids, target_view, _) in enumerate(val_loader):
                        with torch.no_grad():
                            ...
                            target_view = target_view.to(device)
                            feat, _, text_embeds_s = model(img, instruction, label=target, cam_label=target_cam, view_label=target_view )
